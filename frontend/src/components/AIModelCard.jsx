...
export const AIModelCard = ({ model }) => {
  ...
  const handleAnalyze = async (e) => {
    ...
    // ریست فقط با درخواست صریح کاربر
    setLoading(true);
    setError(null);
    setOutput('');
    setShapData(null);
    ...
    try {
      const payload = {
        data: [inputText, model.id || "Baseline"],
        event_data: null,
        fn_index: 2,
        trigger_id: 12,
      ...
      if (!joinResponse.ok) throw new Error(`Failed to join queue: ${joinResponse.status}`);

      const joinResult = await joinResponse.json();
      if (!joinResult.event_id && !joinResult.error) throw new Error("Failed to get event_id from queue join.");
        ...
        try {
          ...
          switch (message.msg) {
            ...
            case "process_completed":
              ...
              if (message.success && message.output?.data?.length > 0) {
                const formattedOutput = `[EXBERT_REPORT]:\n${message.output.data[0]}`;
                if (message.output.data[1]) setShapData(message.output.data[1]);
                setOutput(formattedOutput);
                startTypingProcess(formattedOutput);
              } else {
                setError(`Processing failed: ${message.output?.error || "Unknown server error."}`);
              }
            ...
            case "estimation": {
              const pos = message.rank !== undefined ? message.rank + 1 : '?';
              const eta = message.rank_eta !== undefined ? message.rank_eta.toFixed(1) : '?';
              const waitMsg = `در صف انتظار (${pos}). زمان تقریبی: ${eta} ثانیه...`;
            ...
            case "close_stream":
              ...
              if (loading && !hasCompletedRef.current) {
                setLoading(false);
                if (!output && !error) setError("ارتباط قطع شد (Stream Closed).");
              }
              break;
  ...
  return (
    <div className="bg-[#111] rounded-2xl border border-[#222] shadow-2xl font-vazir flex flex-col h-full max-h-full">
      ...
      <div className="p-5 flex-grow flex flex-col overflow-y-auto scrollbar-thin scrollbar-thumb-[#333] scrollbar-track-transparent">
        ...
        >
          {loading ? (
            <><Loader2 size={18} className="animate-spin" /> در حال تحلیل عمیق شبکه...</>
          ) : isTyping ? (
            <><Terminal size={18} className="animate-pulse" /> در حال نگارش گزارش...</>
          ) : (
            <><Send size={18} /> شروع ارزیابی هوشمند</>
          )}
        </button>
        ...
        <div className="mt-2 bg-[#0a0a0a] rounded-lg p-4 text-cyan-500 text-sm min-h-[100px] border border-cyan-500/30 overflow-x-auto whitespace-pre-wrap font-mono relative shrink-0">
          ...
          {/* حین تایپ: typedOutput — بعد از تایپ: output پایدار */}
          {isTyping ? typedOutput : output}
          {isTyping && <span className="inline-block w-2 h-4 bg-cyan-500 ml-1 animate-pulse"></span>}
