...
// [اصلاح شد] حذف بررسی VITE_HF_API_TOKEN (مربوط به import.meta) برای رفع هشدار build

import React, { useState, useEffect, useCallback, useRef } from 'react';
import { Loader2 } from 'https://esm.sh/lucide-react@0.395.0'; 
...
// [اصلاح شد] بررسی VITE_HF_API_TOKEN حذف شد تا از خطای 'import.meta' در محیط build 'es2015' جلوگیری شود.
// همانطور که در کد قبلی ذکر شد، این برای یک space عمومی مورد نیاز نیست.
console.log("✅ [AIModelCard] Loaded. (HF token check skipped to prevent build warnings).");
  ...
  const handleModelQuery = async (e) => {
    ...
    try {
        ...
        if (!joinResponse.ok) {
             ...
             // [FIX] رفع خطای رفرنس (response به joinResponse تغییر کرد)
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-5 shadow-inner shadow-cyber-green/10 border border-cyber-green/20 flex flex-col h-full">
      ...
      <form onSubmit={handleModelQuery}>
        <textarea
          value={input}
          // [FIX] خطای تایپی "targe" به "target" اصلاح شد
